  ...
  <div class="marketing-product-list flex-wrapper">
    ...
    <div class="sort-box">
      <div class="sort-content clearfix">
        ...
        <van-button type="primary" size="small" color="rgb(231, 46, 51)" class="right right-button" @click="addProduct" plain v-if="productList.length != 0">添加产品</van-button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapState(["selectedTempProduct", "QRcode"]),
    getPages: function() {
      return Math.ceil(this.total / 10);
    }
  },
  methods: {
    ...mapActions(["SET_SELECTEDTEMPPRODUCT", "SET_SELECTEDPRODUCTQRCODE"]),
    addProduct() {
    ...
    confirm() {
      ...
      if (this.nowSelectedIndex == -1) {
        ...
        if (
          ...
          this.productList[this.nowSelectedIndex].productType
        ) {
          // eslint-disable-next-line no-unused-vars
          var code = new QRCode(this.$refs.qrCodeDiv, {
            text:
              process.env.VUE_APP_FRONT_BASE +
              "/commonProductItem/" +
              this.userid +
              "/" +
              this.productList[this.nowSelectedIndex].productUnique +
              "/" +
              this.productList[this.nowSelectedIndex].productType,
            width: 48,
            height: 48,
          ...
          this.SET_SELECTEDTEMPPRODUCT(this.productList[this.nowSelectedIndex]);
          this.$router.go(-1);
        } else {
          if (this.$route.params.productType == 1) {
            this.$toast("选择失败，该模板仅用于理财产品");
          } else if (this.$route.params.productType == 2) {
            this.$toast("选择失败，该模板仅用于基金产品");
          }
        }

        //  console.log("选中产品1", process.env.VUE_APP_BASE_API)
        console.log("选中产品1", this.productList[this.nowSelectedIndex]);
      }
    },
    loadMore() {
      console.log(1);

  ...
  <div class="flex-wrapper">
    ...
    <section @touchend="touchEndTime()">
      ...
      <v-peoplecard :userid="this.$route.params.userid"
                    ref="peoplecard"
                    :isWXApplet="this.$route.params.isWXApplet" versionStyle="2"></v-peoplecard>
      <!-- 译会产品推荐 -->
      ...
      <v-famousremark versionStyle="2"></v-famousremark>
      <!-- 瑞丰产品推荐 -->
      <v-rfproduct :userid="this.$route.params.userid"
                   :openid="this.$route.params.openid"
                   versionStyle="2"></v-rfproduct>
      <!-- 瑞丰产品推荐 -->
      <v-njproduct :userid="this.$route.params.userid"
                   :openid="this.$route.params.openid"
                   versionStyle="2"></v-njproduct>
      <!-- 头条推荐 -->
      <v-firstrecomment30 :todaytime="this.$route.params.todaytime"
                        :userid="this.$route.params.userid"
                        :historyid="this.$route.params.historyid"
                        ref="firstrecomment"
                        :isWXApplet="this.$route.params.isWXApplet"></v-firstrecomment30>
      <!--金融市场-->
      <v-financeMarketsNews30
            :todaytime="this.$route.params.todaytime"
            :historyid="this.$route.params.historyid"
            ref="financeMarketsNews30"
            :isWXApplet="this.$route.params.isWXApplet"
            versionStyle="2"></v-financeMarketsNews30>
      <!--谈天说地-->
      <v-talkEverythingNews30
              :todaytime="this.$route.params.todaytime"
              :historyid="this.$route.params.historyid"
              ref="talkEverythingNews30"
              :isWXApplet="this.$route.params.isWXApplet"
              versionStyle="2"></v-talkEverythingNews30>
      ...
      <!-- 地域新闻 -->
      <v-localnews30 :place="this.$route.params.place"
                   :userid="this.$route.params.userid"
                   ref="localnews"
                   versionStyle="2"></v-localnews30>

      <!-- 地产聚焦 -->
      <v-housenews30 :todaytime="this.$route.params.todaytime"
                     :historyid="this.$route.params.historyid"
                     ref="housenews"
                     :isWXApplet="this.$route.params.isWXApplet"
                     versionStyle="2"></v-housenews30>
      <!--财经百科-->
      <v-financeEconomicsNews30
              :todaytime="this.$route.params.todaytime"
              :historyid="this.$route.params.historyid"
              ref="financeEconomicsNews30"
              :isWXApplet="this.$route.params.isWXApplet"
              versionStyle="2"></v-financeEconomicsNews30>
      <!-- 猜您喜欢 -->
      <!--<v-maylike :openid="this.$route.params.openid"
                 :userid="this.$route.params.userid"
                 :todaytime="this.$route.params.todaytime"></v-maylike>-->
      <!-- 快讯简报 -->
      <v-fast versionStyle="2"></v-fast>
      <!-- 底部行情概览和声明 -->
      <!-- 行情概览 -->
      <v-stock :todaytime="this.$route.params.todaytime" versionStyle="2"></v-stock>
      <v-browsenum :browseNum="browseNum"></v-browsenum>
      <v-bottomfame :isWXApplet="this.$route.params.isWXApplet" versionStyle="2"></v-bottomfame>
      <!-- 底部转发和模块编辑 -->
    ...
    <v-firstComing30></v-firstComing30>
    <!-- 是否可以删除 -->
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      'v-localnews30': () => import(`../../components/common/financialReport30/localNews30`),
      ...
      'v-housenews30': () => import(`../../components/common/financialReport30/houseNews30`),
      ...
      'v-firstrecomment30': () => import(`../../components/common/financialReport30/firstRecomment30`),
        'v-financeEconomicsNews30':()=> import(`../../components/common/financialReport30/financeEconomicsNews30`),
        'v-financeMarketsNews30':()=> import(`../../components/common/financialReport30/financeMarketsNews30`),
        'v-talkEverythingNews30':()=> import(`../../components/common/financialReport30/talkEverythingNews30`),
      'v-famousremark': () => import(`../../components/common/financialReport/famousRemark`),
      'v-peoplecard': VPeoplecard,
      'v-product': VProduct,
      'v-activaty': VActivity,
        'v-firstComing30': () => import(`../../components/common/financialReport30/firstComing30`),
    },
    provide: function () {
      return {
    ...
    computed: {
      ...
      ...mapState(['creditCity', 'shareArticleId', 'orgnCode', 'moduleD',
        ...
        'financeActivityId', 'commentary','productType','hotTodayList30','CommentArticleIndex',
          'moduleM','moduleN','moduleO','financeMarketList','talkEverythingList','financeEconomicsList']),
      //文章号
      fileId: function () {
        if (this.fieldid != -1) {
          return this.fieldid;
    ...
    methods: {
        openFirstComing(){
            this.SET_FIRSTCOMINGSTATE(true);
        },
      //转发获客
      shareToCustomer() {
        if (this.userid != -1) {
      ...
      ...mapActions(['SET_SHAREARTICLEID', 'SET_ORGNCODE', 'SET_MODULEH', 'SET_MODULEK',
        ...
        'SET_FINANCEACTIVITY', 'SET_SKIN', 'SET_REDPACKETID','SET_FIRSTCOMINGSTATE','SET_HOTTODAYLIST30',
          'SET_MODULEM','SET_MODULEN','SET_MODULEO','SET_FINANCEMARKETLIST','SET_TALKEVERYTHINGLIST','SET_FINANCEECONOMICSLIST']),
      //今日财经
      getTodayNews() {
        //分享记录获取
        let parames = {
      ...
      getTitleAbstract(versionCode) {
        ...
        if (ua.match(/MicroMessenger/i) == "micromessenger" && this.isWXApplet != 1) {
          ...
          var modulesTitle = [];
          //头条推荐
          if(this.hotTodayList30.length!=0){
            if (this.moduleC && modulesTitle.join().length + this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title.length < 200) {
              modulesTitle[0] = (this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title);
            } else {
              modulesTitle[0] = '';
            }
          }
          ...
          //金融市场
            if(this.financeMarketList.length!=0){
              if (this.moduleM && this.financeMarketList != [] && modulesTitle.join().length + this.financeMarketList[0].title.length < 200) {
                  modulesTitle[1] = this.financeMarketList[0].title;
              } 
            }else {
                modulesTitle[1] = '';
            }
            //谈天说地
            if(this.talkEverythingList.length!=0){
              if (this.moduleN && this.talkEverythingList != [] && modulesTitle.join().length + this.talkEverythingList[0].title.length < 200) {
                  modulesTitle[2] = this.talkEverythingList[0].title;
              } 
            }
            else {
                modulesTitle[2] = '';
            ...
            //财经百科
            if(this.financeEconomicsList.length!=0){
              if (this.moduleO && this.financeEconomicsList != [] && modulesTitle.join().length + this.financeEconomicsList[0].title.length < 200) {
                  modulesTitle[3] = this.financeEconomicsList[0].title;
              } 
            }
            else {
               modulesTitle[3] = '';
          ...
          var getCommentary="";
          if(this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].userComment!=null){
              getCommentary=this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].userComment.commentary;
          }
          var secondAbstract = [this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title,getCommentary ];
          if (versionCode == "2") {
            var shareAbstract = {
        ...
        if (this.deleteproductid != "") {
          let parame6 = {
            ...
            data: {
              ...
              otherId: this.hotTodayList30[this.CommentArticleIndex].id
            }
          };
          this.$post(JSON.stringify(parame6)).then(result => {
            if (result.errorCode == ERROR_CODE) {
                console.log("删除结构",result);
              Toast.success('删除成功');
              this.$nextTick(() => {
          ...
          if (result.errorCode == ERROR_CODE && result.data.codeType != '400') {
            if (this.isWXApplet != 1) {
              ...
              // this.SET_HOTTODAYRES(result.data.newWorldWideList.C[0]);
                var sendObj=result.data.newWorldWideList.C[0];
                sendObj.userComment= result.data.newUserComment;
                this.SET_HOTTODAYLIST30([sendObj]);
              // this.SET_HOTTODAYLIST30(
              //     result.data.newWorldWideList.C
              // );
                //点赞和评论
              this.$nextTick(() => {
              ...
              // this.SET_COMMENTARY();
                ...
                this.SET_FINANCEMARKETLIST(result.data.newWorldWideList.M);
                this.SET_TALKEVERYTHINGLIST(result.data.newWorldWideList.N);
                this.SET_FINANCEECONOMICSLIST(result.data.newWorldWideList.O);
              //宏观经济
      ...
      showModule(str) {
        ...
        this.SET_MODULEK(RegExp(/K/).test(str));
          ...
          this.SET_MODULEM(RegExp(/M/).test(str));
          this.SET_MODULEN(RegExp(/N/).test(str));
          this.SET_MODULEO(RegExp(/O/).test(str));
      },
      //模块控制
      PartState() {
        if (this.modules == '') {
          ...
          let AbcArr = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L','M','N','O'];
          let moduleArr = [this.$refs.peoplecard.skin, this.financeProductId, this.$refs.firstrecomment.moduleCId
            , 0, 0, 0
            , 0, this.moduleHId, this.moduleIId
            , this.moduleJId, this.moduleKId, this.financeActivityId
              ,this.$refs.financeMarketsNews30.moduleMId,this.$refs.talkEverythingNews30.moduleNId
              ,this.$refs.financeEconomicsNews30.moduleOId];
          let AllModuleId = '';
          let placeArr = [];
          ...
          let ObjectModule = {
            ...
            title: this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title,
            abstractStr: this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].abstractStr,
      ...
      wxRegCallback() {
        ...
        if (this.creditCity != '' && this.weather != '') {
          addTitle = nowMonth + '.' + nowDay + ' | 今日财经 | ' + this.creditCity + '市 ' + this.weather + ' | ' + this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title;
        }
        else {
          addTitle = nowMonth + '.' + nowDay + ' | 今日财经 | ' + this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].title;
        }
        var shareUrl = 'https://' + process.env.VUE_APP_LINK + '.yihuisoft.com/' + process.env.VUE_APP_FRONT + '/#/see30/' + this.todaytime + '/' + this.userid + '/' + this.shareArticleId + "/" + this.openid + '/' + this.nowDate + "/T10";
        var parameObj = {
          ...
          desc2: this.hotTodayList30[this.$refs.firstrecomment.recommentSelectedIndex].abstractStr, // 分享描述, 请自行替换
    ...
    mounted() {
      ...
      window.openFirstComing=this.openFirstComing;
      //微信中插入分享记录
      setTimeout(() => {

  ...
  <div v-if="moduleN">
    <div class="hotText" :class="{'hotText-30':versionStyle==2}">
      <div class="hotTextTitle clearfix" :class="{'hotTextTitle-30':versionStyle==2}">
        ...
        <div class="left">谈天说地</div>
        <div class="right"
             v-if="!isWeixin && historyid!=1">
          <!--<button @click="changeHotText('hot')">换一组</button>-->
          <div class="add-button" @click="addItem" v-if="talkEverythingList.length<5&&userid!=-1">添加</div>
        </div>
      </div>
      <div v-for="(item,index) in talkEverythingList" :key="index" >
        <div class="hotTextList" :class="{'hotTextList-30':versionStyle==2}"
          ...
          <div @click="checkArticle(item.id,item.title,item)">
            <div>
              ...
              <!-- <div class="image-box"> 
                <ul class="image_list">
                  <li class="image_item"><img src="https://p3.pstatp.com/list/pgc-image/RuZTb137lT0yxP" alt=""></li>
                  <li class="image_item"><img src="https://p3.pstatp.com/list/pgc-image/RuZTb2TGaxTqB7" alt=""></li>
                  <li class="image_item"><img src="https://p3.pstatp.com/list/pgc-image/RuZTb357DWu6Aj" alt=""></li>
                </ul>
              </div> -->
            </div>
            <!-- <div v-else-if="index%2==1">
              ...
              <div class="list_img_holder"> 
                  <img src="https://p3.pstatp.com/list/pgc-image/Rv4mHy7FA8oOIV">
              </div>
            </div> -->
          </div>
          <v-lable30 :lableList='item.tagList' :itemIndex='index' versionStyle="2" :showCloseIcon="talkEverythingList.length>2&&userid!=-1"></v-lable30>
        </div>
      </div>
      <div class="warming-tip" v-if="!showshare" :class="{'warming-tip-30':versionStyle==2}">推荐内容已根据您收藏的产品进行优化</div>
      <div v-if="talkEverythingList==false" class="no-news-tips">暂无新闻</div>
      <div class="bottom-trans-button" @click="recommentState=!recommentState"  v-if="!showshare&&talkEverythingList.length>2">
  ...
  export default {
    ...
    computed: {
      ...
      //   ...mapState(['moduleG', 'hotMarketList']),
        ...mapState(['moduleN','talkEverythingList']),
      //时间转化
      formatDate: function () {
    ...
    created() {
      ...
      if (ua.match(/MicroMessenger/i) == "micromessenger" && this.isWXApplet != 1) {
        ...
        //谈天说地
        this.initTalkEverything();
      }
    },
    methods: {
      //Vuex辅助函数，将公共状态中的方法映射成当前的this方法
      ...mapActions(['SET_TALKEVERYTHINGLIST']),
            ...
            let parame =
                {
                    ...
                    "data": {
                        "id":this.talkEverythingList[index].id,
                        "userId": this.userid,
                        "modelType":this.modelType
                    }
                }
      ...
      //初始化谈天说地
        ...
        this.$post(JSON.stringify(parame)).then(result => {
            console.log("==>谈天说地参数", parame);
            console.log("==>谈天说地结果", result);
          if (result.errorCode == ERROR_CODE && result.data.codeType != '400') {
            this.SET_TALKEVERYTHINGLIST(result.data.chitChatList);
            this.modelType=result.data.modelType;
                let getString = '';
                for (var i = 0; i < result.data.chitChatList.length; i++) {
                  getString += result.data.chitChatList[i].id + ',';
                }
                this.moduleNId = getString.substring(0, getString.length - 1);
          }
